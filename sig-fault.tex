...
With respect to the first part, besides $k$ times that it is needed to be called to detect the faulty schemas, it still needs more test cases to ensure that the remaining schemas are not faulty. This number is no more than the number of schemas in $CMXS(FSS^{\bot}, t)$, in which the schemas in this set are candidate maximal pending schemas. According to the definition of CMXS, this number is no more than $d^{2}$, where $d$ is the degree of the MFS. As these schemas are healthy schemas, \emph{Binary-Search-Check} function only needs 1 additional test case (Only check the first schema in the list). Finally, there is $k \times log_{2}n +  d^{2} $ in total for the second part.

At last, the number of test cases generated by the first part and second part is no more than $k \times (d - 1) \times log_{2}n$ + $k \times log_{2}n$ + $d^{2} $ =
$k \times d \times log_{2}n + d^{2} $.  Append the additional test cases for re-check each MFS (For alleviation of assumption 3), the total number needed by our algorithm is no more than $k \times d \times log_{2}n + d^{2} + d $.
...
\begin{table*}[!htb]
  ...
  \hline
    ...
    FIC\_BS & $\leq$ $k \times (d \times (log_{2}(n) + 1))$ & 1$\sim$n & multiple, none overlapped  & $\oslash$ \\
    LG & $k \times log_{2}(n) + k^{2}$  & 2 &  multiple & $\oslash$ \\
    SP & $\geq$ n  & $\leq$ d & multiple & $\odot$\\
    CTA & n  & 1$\sim$n  & single & $\oslash$  \\
    RI & $ k \times (d \times (log_{2}(n) + 1))$  & 1$\sim$n &  multiple, none overlapped  & $\oslash$  \\
    AIFL & $\leq$ $2^{n}$  & 1$\sim$n & multiple, none overlapped  & $\oslash$  \\
    TRT & $k \times d \times log_{2}n + d^{2}$  & 1$\sim$n & multiple & $\oslash$  \\
    TRTNA   & $3 (\times k \times d \times log_{2}n + d^{2})$ & 1$\sim$n & multiple & $\odot$  \\

      \hline
    CMS & $k \times d \times log_{2}n + d^{2} + d$ & 1$\sim$n & multiple   &  $\odot$ \\
